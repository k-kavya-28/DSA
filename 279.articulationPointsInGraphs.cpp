...
vector<int>& ap, int &timer){
    ...
    for(auto nbr: adj[node]){
        ...
        if(!vis[nbr]){
            ...
            //aake low update krte hain as if child ka low update ho gya h toh mera bhi hoga
            low[node] = min(low[node], low[nbr]);
            //check ap or not
            if(low[nbr] >= disc[node] && parent != -1){
                ap[node] = 1;
            }
            //not visited wala case tha isliye child++ krdiya, dfs me number of connected components bhi nikal skte hain
            child++;
        }
        else{
            //back edge wala case
...
int main()
{   
    int n = 5;  //the example in notebook
    int e = 5;
    ...
    //initialising these data structures
    for(int i=0; i<n; i++){
        disc[i] = -1;
        low[i] = -1;
    }
